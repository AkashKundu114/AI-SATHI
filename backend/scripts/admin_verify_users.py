    ...
    async with get_db_session() as db:
        ...
        for uv, user in records:
            ...
            await db.execute(
                ...
                .values(status=status, reviewed_at=datetime.utcnow(), reviewer_notes=notes)
            )
            await db.execute(update(User).where(User.id == user.id).values(verification_status=user_status))
            await db.commit()
