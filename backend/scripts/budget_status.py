...
async def _budget_rows() -> list[dict]:
    async with get_db_session() as db:
        rows = (
            await db.execute(text("SELECT vendor, used, total_budget, degraded_mode, hard_stopped FROM credit_budget"))
        ).fetchall()
    return [
        {
            "vendor": r[0],
            ...
            "degraded_mode": bool(r[3]),
            "hard_stopped": bool(r[4]),
        }
        for r in rows
    ]
...
async def main() -> None:
    ...
    any_alert = any(
        b["hard_stopped"]
        or b["degraded_mode"]
        or (b["used"] / b["total_budget"] if b["total_budget"] else 0) >= ALERT_BUDGET_CONSUMED_FRACTION
        for b in budgets
    )
    if any_alert:
