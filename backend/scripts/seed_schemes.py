...
async def _seed_one_pdf(pdf_path: Path, meta: dict, *, force: bool, dry_run: bool) -> dict:
    ...
    async with get_db_session() as db:
        existing = (await db.execute(sql_text(_EXISTING_ACTIVE_SQL), {"source_file": pdf_path.name})).fetchone()
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Extract and chunk only - no DB writes, no embedding calls"
    )
    parser.add_argument(
        "--only", type=str, default=None, help="Process a single PDF filename instead of every PDF in data/schemes/raw/"
    )
    parser.add_argument(
        "--force",
        action="store_true",
