...
from shared.observability.email_reporter import send_metrics_email, render_metrics_html_report
...
async def main_async(to_email: str | None, dry_run: bool, json_output: bool) -> None:
    ...
    if dry_run:
        ...
        print(f"  * Estimated Cost    : INR {metrics['api_usage']['estimated_cost_inr']} (~${metrics['api_usage']['estimated_cost_usd']})")
        return
...
def main() -> None:
    ...
    parser.add_argument("--dry-run", action="store_true", help="Collect and render report locally without sending email")
