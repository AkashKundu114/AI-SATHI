...
async def collect_system_metrics() -> dict:
    ...
    async with get_db_session() as db:
        try:
            ...
            onboarded_count = (
                await db.execute(text("SELECT COUNT(*) FROM users WHERE consent_given = TRUE"))
            ).scalar() or 0
            new_24h = (
                await db.execute(text("SELECT COUNT(*) FROM users WHERE onboarded_at >= NOW() - INTERVAL '24 hours'"))
            ).scalar() or 0
            ...
            usage_rows = (
                await db.execute(
                    text(
                        "SELECT provider, SUM(call_count) FROM api_usage_monthly WHERE month_bucket = :m GROUP BY provider"
                    ),
                    {"m": month_bucket},
                )
            ).fetchall()
            ...
            metrics["api_usage"]["estimated_cost_usd"] = round((sarvam_cost_inr / 83.5) + flux_cost_usd, 2)

            active_rate_limits = (
                await db.execute(
                    text(
                        "SELECT COUNT(DISTINCT phone_number) FROM rate_limit_counters WHERE hour_bucket >= (EXTRACT(EPOCH FROM NOW()) / 3600 - 24)::BIGINT"
                    )
                )
            ).scalar() or 0
