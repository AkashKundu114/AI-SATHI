...
PHRASING_SYSTEM = (
    "তুমি একজন বন্ধুত্বপূর্ণ মূল্য নির্ধারণ পরামর্শদাতা।\n\n"
    f"{DIGNITY_RULES_BENGALI}\n\n"
    "দেওয়া সংখ্যার ভিত্তিতে\n"
...
async def pricing_node(state: ConversationState) -> dict:
    ...
    if calc["floor_price"] <= 0:
        return {
            "outbound_messages": [{"type": "text", "body": NO_PROFILE_MSG}],
            "trace": ["pricing_node:non_positive_floor"],
        }
    ...
    prompt = (
        ...
        f"বাজারের গড় দাম: {'₹' + str(round(market_avg)) if market_avg else '정보 নেই'}"
    )
    try:
