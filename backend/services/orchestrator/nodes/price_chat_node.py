...
from services.orchestrator.model_router import AgentTier, ModelUnavailableError, TaskCriticality, route_completion
...
async def price_chat_node(state: ConversationState) -> dict:
    ...
    if not profile or not profile.production_cost:
        return {
            "outbound_messages": [
                {"type": "text", "body": "আগে তৈরির খরচ ও সর্বনিম্ন দাম জানান, তারপর একসাথে বসে দাম ঠিক করব।"}
            ],
    ...
    if floor <= 0:
        return {
            "outbound_messages": [
                {"type": "text", "body": "তৈরির খরচের তথ্যে একটু সমস্যা মনে হচ্ছে -- আবার 'দাম' লিখে ঠিক করে জানান।"}
            ],
    ...
    if signal == "wants_lower":
        ...
        reason = await _generate_reply(
            f"বিক্রেতা দাম আরেকটু কমাতে চাইছেন। নতুন প্রস্তাব: ₹{new_proposed:.0f} (এই সংখ্যাটা বলো না)। কেন এটা যুক্তিসঙ্গত এক লাইনে বলো।"
        )
        body = await _verified_body(
