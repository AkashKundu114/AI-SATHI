...
from shared.guardrails.response_cache import build_cache_key, get_cached_response, set_cached_response
...
MARKET_CACHE_TTL_SECONDS = 20 * 60
...
async def market_predictor_node(state: ConversationState) -> dict:
    ...
    report["crops_at_harvest"] = [
        {"slug": c.slug, "name_bengali": c.name_bengali, "note": c.note_bengali} for c in this_month_harvest
    ]
    ...
    cache_signature = (
        f"rising={sorted(report['rising'])};saturated={sorted(report['saturated'])};"
        f"festivals={[f['slug'] for f in knowledge['upcoming_festivals']]};"
        f"crops={[c['slug'] for c in report['crops_at_harvest']]}"
    )
    cache_key = build_cache_key("market_report", cache_signature, scope=block)

    cached = await get_cached_response(cache_key, ttl_seconds=MARKET_CACHE_TTL_SECONDS)
    if cached:
        return {
            "market_report": report,
            "outbound_messages": [{"type": "text", "body": cached}],
            "trace": [
                f"market_predictor_node:cache_hit:rising={len(report['rising'])}:saturated={len(report['saturated'])}"
            ],
        }

    try:
        phrased = await _phrase_report(report)
        await set_cached_response(cache_key, phrased)
...
async def _phrase_report(report: dict) -> str:
    ...
    prompt = (
        ...
        f"বেসি সরবরাহ থাকা পণ্য: {', '.join(report['saturated']) or 'নেই'}\n"
