...
from shared.guardrails.poster_dedup import build_poster_dedup_key, get_cached_poster, record_poster
from shared.storage.blob_client import download_bytes, generate_read_url, upload_bytes
...
async def catalog_node(state: ConversationState) -> dict:
    ...
    if user_id:
        under_cap = await check_and_increment_daily_feature_cap(user_id, "catalog", MAX_CATALOG_CREATIONS_PER_DAY)
        if not under_cap:
            return {
    ...
    return {
        **state_clears,
        ...
        "trace": [
            f"catalog_node:done:{product_info.get('vision_model_used')}:poster={poster_tier}:agreed_price={agreed_price is not None}"
        ],
    }


async def _build_delivery_messages(
    processed_bytes, processed_key, product_info, captions, price_min, price_max, market_note, state
):
    ad_caption_full = captions["ad_caption"] + (f"\n{market_note}" if market_note else "")
    product_label = _product_label_bengali(product_info)
    user_id = state.get("user_id")

    if user_id:
        dedup_key = build_poster_dedup_key(user_id, product_label, ad_caption_full)
        cached = await get_cached_poster(dedup_key)
        if cached:
            try:
                ...
                return (
                    ...
                    f"{cached['poster_tier']}-cached",
                )
            except Exception:
                logger.warning("cached poster URL generation failed, falling through to a fresh generation")
