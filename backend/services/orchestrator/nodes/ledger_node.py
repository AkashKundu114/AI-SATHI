...
RURAL_ITEM_PATTERNS =[
(re .compile (r'\b(sorser|sorse|shorsher|shorshe|soriser|mustard|সর্ষের?|সরিষার?|সরিষা)\s*(tel|তেল)?\b',re .I ),"সর্ষের তেল"),
...
def _extract_single_clause (clause :str )->dict |None :
    ...
    "beej","bij","বীজ","tel","তেল","bhara","ভাড়া","bill","বিল","recharge","রিচার্জ","legeche","লেগেছে","niyeche","নিয়েছে",
...
async def ledger_extract_node (state :ConversationState )->dict :
    ...
    if len (nums_in_orig )==1 and len (transactions )==1 :
        ...
        if curr_amt !=explicit_num and not any (w in original_transcript .lower ()for w in ["duto","tinte","charte","hali","jora","kg","কেজি","দুটো","তিনটে"]):
