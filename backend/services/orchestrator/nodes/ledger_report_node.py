...
async def ledger_report_node(state: ConversationState) -> dict:
    ...
    under_cap = await check_and_increment_daily_feature_cap(user_id, "report", MAX_LEDGER_REPORTS_PER_DAY)
    if not under_cap:
        return {
    ...
    if result["total_income"] == 0 and result["total_expense"] == 0:
        return {
            "outbound_messages": [
                {"type": "text", "body": f"{GREGORIAN_MONTHS_BENGALI[today.month]} মাসে কোনো হিসাব পাওয়া যায়নি।"}
            ],
