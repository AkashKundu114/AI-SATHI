...
async def _apply_correction (state :ConversationState ,pending :dict ,correction_text :str ,turns :int )->dict :
    from services .orchestrator .nodes .ledger_node import _build_confirmation ,_extract_single_clause ,_strip_json_fences 
    ...
    try :
        ...
        try :
            ...
            if txns :
                ...
                "ledger_confirmation_turns":turns ,
                "outbound_messages":[
                {"type":"text","body":f"আমি আপনার হিসাবটি সংশোধন করেছি। আপনি কি নিশ্চিত করতে চান যে:\n{_build_confirmation (updated )}\n\nএই হিসাবটি কি ঠিক আছে? (হ্যাঁ/না)"}
                ],
                "trace":[f"ledger_confirm_node:correction_applied:{result .get ('model_used')}:turn={turns }"],
                }
            else :
                return _reset_with_message ("সংশোধন বুঝতে পারলাম না। অনুগ্রহ করে আবার বলুন।",trace =f"ledger_confirm_node:malformed_json:turn={turns }")
        except Exception :
            return _reset_with_message ("সংশোধন বুঝতে পারলাম না। অনুগ্রহ করে আবার বলুন।",trace =f"ledger_confirm_node:malformed_json:turn={turns }")
        ...
        return _reset_with_message ("এই মুহূর্তে সংশোধন প্রসেস করতে সমস্যা হচ্ছে। একটু পরে আবার চেষ্টা করুন।",trace =f"ledger_confirm_node:model_unavailable:turn={turns }")
