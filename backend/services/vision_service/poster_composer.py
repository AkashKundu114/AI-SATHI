...
from services.vision_service.flux_poster_client import FluxUnavailableError, generate_poster_image
from shared.config.feature_caps import CREDIT_COST_FLUX_POSTER
from shared.config.settings import get_settings
from shared.guardrails.budget import charge_credits, is_hard_stopped
    ...
    *,
    ...
    try:
        ...
        if shg_name:
            ...
            draw.text(
                (w - wm_w - _PADDING, banner_h - body_font.size - 12), watermark, font=body_font, fill=_TEXT_COLOR
            )
...
async def generate_poster(
    ...
    *,
    ...
    flux_available = bool(s.flux_api_key)
    if flux_available and await is_hard_stopped("flux"):
        logger.info("Flux budget hard-stopped - skipping Flux tier, using free Pillow composite")
    ...
    if flux_available:
        try:
            ...
            await charge_credits("flux", CREDIT_COST_FLUX_POSTER, "poster", user_id=user_id)
