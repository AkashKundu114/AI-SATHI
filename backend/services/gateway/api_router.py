...
async def process_chat_message(
    payload: dict[str, Any], current_user_phone: str = Depends(get_current_user_phone)
) -> dict:
    """
    ...
    try:
        ...
        reply = (
            guard_res.canned_reply
            if guard_res.canned_reply
            else "Your request has been received. Speak or write for ledger and market assistance."
        )
        return {
            "status": "success",
...
async def parse_ledger_from_text(
    payload: ParseRequest, current_user_phone: str = Depends(get_current_user_phone)
) -> dict:
    """
    ...
    try:
        ...
        return {
            ...
            "ai_message": "আপনার বার্তা থেকে এই লেনদেন চিহ্নিত করা হয়েছে:"
            if parsed_entries
            else "আপনার বার্তা বিশ্লেষণ করা হয়েছে।",
            "raw_text": sanitized,
        }
    except Exception as exc:
...
async def confirm_and_save_ledger(
    payload: ConfirmLedgerRequest, current_user_phone: str = Depends(get_current_user_phone)
) -> dict:
    """
    ...
    try:
        ...
        if not transcript:
            return {
                ...
                "messages": [
                    {
                        "type": "text",
                        "body": "ভয়েস রেকর্ডটি ঠিকমত বোঝা যায়নি। অনুগ্রহ করে একটু স্পষ্ট করে আবার বলুন। (Could not transcribe voice note. Please speak clearly again.)",
                    }
                ],
        ...
        latest_msg = (
            ...
            if outbound
                ...
                "type": "text",
                "body": f'✅ Voice note recorded: "{transcript}"\n\nLedger and market advice updated.',
            }
        )
        if latest_msg.get("type") == "text" and latest_msg.get("body"):
            latest_msg["body"] = _format_multi_coherence_response(latest_msg["body"])
                ...
                doc_record = (
                    await db.execute(
                        select(UploadedDocument).where(
                            UploadedDocument.user_phone == phone, UploadedDocument.filename == doc_name
                        )
                    )
                ).scalar_one_or_none()
                if doc_record:
...
async def upload_attachment_to_azure(
    container: str = Form("chat-media"), user_phone: str = Form(None), file: UploadFile = File(...)
):
    try:
        content = await file.read()
        from shared.storage.azure_client import upload_azure_blob

        blob_url = await upload_azure_blob(
            container, file.filename or "attachment.bin", content, file.content_type or "application/octet-stream"
        )
                ...
                async with get_db_session() as db:
                    stmt = (
                        ...
                        .values(
                            ...
                            text_content=text,
                            blob_url=blob_url
                            or f"https://aisathistorage.blob.core.windows.net/{container}/{file.filename}",
                        )
...
async def get_ledger_entries(phone: str, current_user_phone: str = Depends(get_current_user_phone)) -> dict:
    ...
    try:
        ...
        async with get_db_session() as db:
            user = await _get_or_create_user(db, phone)
            entries_query = (
                select(LedgerEntry).where(LedgerEntry.user_id == user.id).order_by(LedgerEntry.entry_date.desc())
            )
            ...
            return {"status": "success", "entries": result_entries}
    except HTTPException:
        raise
    except SQLAlchemyError as exc:
...
async def add_ledger_entry(
    payload: LedgerEntryCreate, current_user_phone: str = Depends(get_current_user_phone)
) -> dict:
    """
...
async def delete_ledger_entries(
    phone: str, entry_id: str | None = None, current_user_phone: str = Depends(get_current_user_phone)
) -> dict:
    """
        ...
        async with get_db_session() as db:
            ...
            if entry_id:
                try:
                    ...
                    stmt = delete(LedgerEntry).where((LedgerEntry.user_id == user.id) & (LedgerEntry.id == eid))
                except ValueError:
                    stmt = delete(LedgerEntry).where((LedgerEntry.user_id == user.id) & (LedgerEntry.id == entry_id))
            else:
                stmt = delete(LedgerEntry).where(LedgerEntry.user_id == user.id)

            res = await db.execute(stmt)
            await db.commit()
            return {"status": "success", "message": f"Deleted {res.rowcount} entry/entries"}
    except HTTPException:
        raise
    except Exception as exc:
...
async def list_user_documents(phone: str, current_user_phone: str = Depends(get_current_user_phone)) -> dict:
    ...
    try:
        ...
        async with get_db_session() as db:
            docs = (
                (await db.execute(select(UploadedDocument).where(UploadedDocument.user_phone == phone))).scalars().all()
            )
            result = []
            for doc in docs:
