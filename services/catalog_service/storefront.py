...
async def get_seller_catalog_storefront(user_id: str, base_url: str = "https://kothakhata.app") -> dict:
    """Aggregates all catalog items for a seller to generate a WhatsApp digital storefront."""
    if not user_id:
    ...
    async with get_db_session() as db:
        rows = (
            await db.execute(
                text(
                    """
                    SELECT id, product_type, caption_bengali, ad_caption_bengali, price_min, price_max, s3_url, created_at
                    FROM catalog_creations
                    WHERE user_id = :uid
