...
async def collect_system_metrics() -> dict:
    """Collects comprehensive application, database, usage, and cost metrics."""
    ...
    async with get_db_session() as db:
        try:
            ...
            # Free users not in user_plans table
            ...
            # Sarvam Chat ~₹0.012 per call (1200 avg tokens)
            # Sarvam Vision ~₹0.50 per page
            # Sarvam STT ~₹0.25 per voice note (30s avg)
            # Sarvam Translate ~₹0.01 per call
            # Flux Pro ~$0.04 per image (~₹3.35)
            sarvam_cost_inr = (
