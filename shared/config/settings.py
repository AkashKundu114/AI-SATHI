...
class Settings(BaseSettings):
    ...
    # --- WhatsApp Flows (Meta Business verification required, same as
    # docs/archive/product/trd.md's scheme_eligibility_flow.json). Optional —
    # if unset, ledger confirmation falls back to the existing plain-text
    # হ্যাঁ/না flow (ledger_confirm_node.py), which still works and needs no
    # Flow at all. See shared/whatsapp/sender.py:send_flow for the
    # verification caveat on the exact message payload shape. ---
    ...
    # Referenced by services/rag_service/pipeline.py:get_embedding() and
    # scripts/seed_schemes.py — was missing from this file entirely before
    # this pass, a latent bug (get_settings().ollama_embedding_model would
    # have raised AttributeError the moment either of those functions ran).
    # nomic-embed-text is the model docs/architecture.md and llm-guide.md
    # both already assume for the free local-embeddings tier.
