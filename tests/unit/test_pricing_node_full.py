...
async def test_no_seller_profile_row_returns_no_profile_message(monkeypatch):
    monkeypatch.setattr(
        node_module, "get_db_session", _fake_get_db_session(profile=None)
    )
...
async def test_profile_with_no_production_cost_returns_no_profile_message(monkeypatch):
    monkeypatch.setattr(
        node_module,
        "get_db_session",
        _fake_get_db_session(profile=_profile(production_cost=None)),
    )
    result = await node_module.pricing_node({"user_id": "u1"})
    assert result["trace"] == ["pricing_node:no_profile"]
...
@pytest.mark.asyncio
async def test_negative_cost_with_no_minimum_price_refuses_rather_than_use_zero_floor(
    monkeypatch,
):
    monkeypatch.setattr(
        node_module,
...
async def test_happy_path_with_block_pulls_market_average(monkeypatch):
    ...
    async def _fake_trend(block):
        return [
            {"category": "papad", "total_amount": 1000.0},
            {"category": "papad", "total_amount": 2000.0},
        ]

    async def _fake_completion(**kwargs):
        return {
            "text": "বাজারের গড় দাম অনুযায়ী ভালো।",
    ...
    result = await node_module.pricing_node(
        {"user_id": "u1", "user_profile": {"block": "Balidewanganj"}}
    )
...
async def test_minimum_price_overrides_lower_margin_floor(monkeypatch):
    monkeypatch.setattr(
        ...
        _fake_get_db_session(
            profile=_profile(
                production_cost=100, preferred_margin=0.10, minimum_price=180
            )
        ),
    )
