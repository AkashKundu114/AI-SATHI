...
def _many_transactions(n: int) -> list[dict]:
    return [
        {"type": "INCOME", "amount_inr": 10, "item_bengali": f"item{i}"}
        for i in range(n)
    ]
...
async def test_transaction_list_is_truncated_at_the_cap(monkeypatch):
    async def _fake(**kwargs):
        return {
            "text": json.dumps(
                {"transactions": _many_transactions(50), "confidence": 0.9}
            ),
    ...
    result = await node_module.ledger_extract_node(
        {"raw_input_transcript": "অনেকগুলো লেনদেন"}
    )
    assert (
        len(result["pending_ledger_entry"]["transactions"])
        == node_module.MAX_TRANSACTIONS_PER_ENTRY
    )
...
async def test_transaction_list_under_cap_is_untouched(monkeypatch):
    async def _fake(**kwargs):
        return {
            "text": json.dumps(
                {"transactions": _many_transactions(3), "confidence": 0.9}
            ),
            "model_used": "sarvam-standard",
            "escalated": False,
        }
    ...
    result = await node_module.ledger_extract_node(
        {"raw_input_transcript": "তিনটি লেনদেন"}
    )
