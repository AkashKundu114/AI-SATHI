...
def _user(**overrides):
    base = dict(
        id="uuid-1234", business_categories=["papad", "pickle"], self_reported_literacy="functional",
        preferred_modality="voice", dialect_hint="rarhi", ledger_correction_rate=0.125,
        trust_stage="established", block="Balidewanganj", district="Hooghly",
    )
...
async def test_new_user_not_found_returns_is_new_user_true(monkeypatch):
    ...
    result = await node_module.load_user_profile_node({"whatsapp_number": "919876543210"})
...
async def test_db_error_is_treated_as_new_user_not_a_crash(monkeypatch):
    monkeypatch.setattr(node_module, "get_db_session", _fake_get_db_session(raise_exc=RuntimeError("db down")))
...
async def test_existing_user_returns_full_profile(monkeypatch):
    monkeypatch.setattr(node_module, "get_db_session", _fake_get_db_session(user=_user()))
...
async def test_null_business_categories_defaults_to_empty_list(monkeypatch):
    monkeypatch.setattr(node_module, "get_db_session", _fake_get_db_session(user=_user(business_categories=None)))
...
async def test_null_correction_rate_defaults_to_zero(monkeypatch):
    monkeypatch.setattr(node_module, "get_db_session", _fake_get_db_session(user=_user(ledger_correction_rate=None)))
