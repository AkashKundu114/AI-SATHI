...
async def test_await_name_valid_reply_advances_to_await_block():
    result = await node_module.onboarding_node({"onboarding_step": "AWAIT_NAME", "raw_input_text": "সুনীতা দাস"})
...
async def test_await_name_truncates_absurdly_long_input():
    long_name = "স" * 500
    result = await node_module.onboarding_node({"onboarding_step": "AWAIT_NAME", "raw_input_text": long_name})
    assert len(result["onboarding_name"]) == node_module._MAX_FIELD_LEN


@pytest.mark.asyncio
async def test_await_block_empty_reply_asks_again():
    result = await node_module.onboarding_node({"onboarding_step": "AWAIT_BLOCK", "raw_input_text": ""})
    assert result["trace"] == ["onboarding_node:empty_block"]


@pytest.mark.asyncio
async def test_await_block_valid_reply_advances_to_await_consent():
    result = await node_module.onboarding_node({"onboarding_step": "AWAIT_BLOCK", "raw_input_text": "Balidewanganj"})
...
async def test_await_consent_refusal_does_not_create_user(monkeypatch):
    ...
    result = await node_module.onboarding_node({"onboarding_step": "AWAIT_CONSENT", "raw_input_text": "না"})
...
async def test_await_consent_accepted_creates_user_and_completes(monkeypatch):
    ...
    result = await node_module.onboarding_node({"onboarding_step": "AWAIT_CONSENT", "raw_input_text": "হ্যাঁ"})
...
async def test_await_consent_accepts_english_yes_variants(monkeypatch):
    ...
    for variant in ["ha", "haan", "yes", "হ্যা"]:
        result = await node_module.onboarding_node({"onboarding_step": "AWAIT_CONSENT", "raw_input_text": variant})
