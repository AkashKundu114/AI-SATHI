...
def test_valid_confirmation_choice_passes():
    assert (
        validate_ledger_confirm_payload({"confirmation_choice": "confirm_save"})
        == "confirm_save"
    )
...
def test_unknown_choice_value_rejected():
    assert (
        validate_ledger_confirm_payload({"confirmation_choice": "delete_everything"})
        is None
    )
...
def test_scheme_eligibility_valid_payload_passes():
    payload = {
        "scheme_name": "lakshmir_bhandar",
        "age": "35",
        "has_swasthya_sathi": "yes",
        "is_govt_employee": "no",
    }
