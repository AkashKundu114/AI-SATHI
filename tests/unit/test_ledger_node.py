...
async def test_model_unavailable_falls_back_to_deterministic_extraction(monkeypatch):
    ...
    result = await node_module.ledger_extract_node(
        {"raw_input_transcript": "৩০০ টাকা পাপড় বিক্রি করেছি"}
    )
...
async def test_low_confidence_extraction_asks_to_clarify(monkeypatch):
    ...
    result = await node_module.ledger_extract_node(
        {"raw_input_transcript": "৩০০ টাকা পাপড় বিক্রি"}
    )
    assert result["pending_ledger_entry"] is None
...
@pytest.mark.asyncio
async def test_empty_transactions_list_asks_to_clarify_even_with_high_confidence(
    monkeypatch,
):
    async def _fake(**kwargs):
    ...
    result = await node_module.ledger_extract_node(
        {"raw_input_transcript": "আজ কিছু হয়নি"}
    )
    assert result["pending_ledger_entry"] is None
...
async def test_happy_path_builds_confirmation_and_sets_pending_entry(monkeypatch):
    ...
    result = await node_module.ledger_extract_node(
        {"raw_input_transcript": "৩০০ টাকা পাপড় বিক্রি করেছি, ১০০ টাকা মশলা কিনেছি"}
    )
    assert result["awaiting_confirmation"] is True
...
async def test_strips_markdown_json_fences_before_parsing(monkeypatch):
    ...
    result = await node_module.ledger_extract_node(
        {"raw_input_transcript": "৫০ টাকা মুড়ি বিক্রি"}
    )
    assert result["pending_ledger_entry"] is not None
...
async def test_high_correction_rate_user_gets_a_stricter_confidence_floor(monkeypatch):
    ...
    async def _fake(**kwargs):
        ...
        return _fake_completion_result(
            {
                "transactions": [
                    {"type": "INCOME", "amount_inr": 100, "item_bengali": "x"}
                ],
                "confidence": 0.99,
            }
        )
    ...
    await node_module.ledger_extract_node(
        {"raw_input_transcript": "১০০ টাকা বিক্রি", "user_profile": profile}
    )
...
@pytest.mark.asyncio
async def test_code_mixed_transcript_triggers_translation_before_extraction(
    monkeypatch,
):
    calls = {"translate": 0}
    ...
    async def _fake_translate(text, target_lang):
        calls["translate"] += 1
        return {
            "text": "আজ পাপড় তিনশো টাকায় বিক্রি করেছি",
            "model_used": "sarvam-translate",
        }
    ...
    result = await node_module.ledger_extract_node(
        {"raw_input_transcript": "aj papad tinsho taka bikri korechi"}
    )
    assert calls["translate"] == 1
    assert (
        result["pending_ledger_entry"]["raw_transcript"]
        == "আজ পাপড় তিনশো টাকায় বিক্রি করেছি"
    )
...
async def test_pure_bengali_transcript_never_calls_translation(monkeypatch):
    ...
    result = await node_module.ledger_extract_node(
        {"raw_input_transcript": "আজ পাপড় তিনশো টাকায় বিক্রি করেছি"}
    )
    assert result["pending_ledger_entry"] is not None
