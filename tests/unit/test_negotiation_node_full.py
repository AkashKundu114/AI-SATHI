...
async def test_negative_cost_profile_treated_as_no_profile(monkeypatch):
    monkeypatch.setattr(
        ...
        _fake_get_db_session(
            profile=_profile(production_cost=-500, minimum_price=None)
        ),
    )
...
async def test_starting_with_no_offer_in_text_asks_for_one(monkeypatch):
    monkeypatch.setattr(
        node_module, "get_db_session", _fake_get_db_session(profile=_profile())
    )
    result = await node_module.negotiation_node(
        {"user_id": "u1", "raw_input_text": "কাস্টমার আসতে চায়"}
    )
...
async def test_starting_offer_at_or_above_floor_is_accepted(monkeypatch):
    ...
    result = await node_module.negotiation_node(
        {"user_id": "u1", "raw_input_text": "কাস্টমার ১৫০ টাকা বলেছে"}
    )
...
async def test_starting_offer_below_floor_gets_a_counter(monkeypatch):
    ...
    result = await node_module.negotiation_node(
        {"user_id": "u1", "raw_input_text": "কাস্টমার ৮০ টাকা বলেছে"}
    )
...
async def test_accepting_previous_counter_finalizes_deal(monkeypatch):
    ...
    result = await node_module.negotiation_node(
        {"pending_negotiation": pending, "raw_input_text": "হ্যাঁ"}
    )
    assert result["pending_negotiation"] is None
    assert "130" in result["outbound_messages"][0]["body"]
...
async def test_continuing_with_no_offer_repeats_the_ask():
    ...
    result = await node_module.negotiation_node(
        {"pending_negotiation": pending, "raw_input_text": "চিন্তা করছি"}
    )
    assert result["outbound_messages"][0]["body"] == node_module.NO_OFFER_MSG
...
async def test_exceeding_max_turns_holds_firm_and_ends_negotiation():
    pending = {
        "floor_price": 130.0,
        "product_type": "papad",
        "turns": node_module.MAX_NEGOTIATION_TURNS,
    }
    result = await node_module.negotiation_node(
        {"pending_negotiation": pending, "raw_input_text": "৯০ টাকা দেব"}
    )
    assert result["pending_negotiation"] is None
    assert "130" in result["outbound_messages"][0]["body"]
...
async def test_second_counter_splits_gap_between_floor_and_offer(monkeypatch):
    ...
    result = await node_module.negotiation_node(
        {"pending_negotiation": pending, "raw_input_text": "১০০ টাকা দেব"}
    )
    assert result["pending_negotiation"]["last_counter"] == 200.0


@pytest.mark.asyncio
async def test_model_unavailable_during_reason_generation_still_completes_deal(
    monkeypatch,
):
...
async def test_reason_containing_a_number_is_discarded_not_shown(monkeypatch):
    async def _fake(**kwargs):
        return {
            "text": "৫০ টাকা কম দিলেও চলবে",
            "model_used": "sarvam-advanced",
            "escalated": False,
        }
