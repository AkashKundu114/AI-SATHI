    ...
    async with get_db_session() as db:
        rows = (
            await db.execute(
                text(
                    """
                    SELECT u.whatsapp_number, COALESCE(p.plan_tier, 'free') AS tier, a.provider, a.call_count
                    FROM api_usage_monthly a
                    JOIN users u ON a.user_id = u.id
                    LEFT JOIN user_plans p ON u.id = p.user_id
                    WHERE a.month_bucket = :month
                    ORDER BY u.whatsapp_number, a.provider
                    """
                ),
                {"month": month_bucket},
            )
        ...
        summary = (
            await db.execute(
                text(
                    """
                    SELECT provider, SUM(call_count) as total_calls
                    FROM api_usage_monthly
                    WHERE month_bucket = :month
                    GROUP BY provider
                    """
                ),
                {"month": month_bucket},
