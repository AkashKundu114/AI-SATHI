...
This was previously a stub (`print("RAG audit script — ...")`). This is a
real implementation.

There is currently no persisted log of real user questions asked to the
scheme RAG (the feature isn't even wired into the live intent router yet —
see docs/architecture.md §10.4 / the repo README's scope note), so "random
Q&As" here means: sample the schemes that are actually active in
scheme_documents, ask each a handful of realistic Bengali questions
(benefit amount, eligibility, required documents, timeline), and run them
through the *exact* production path (query_scheme_rag + verify_grounding)
rather than hand-picking favorable examples. Ungrounded results are sorted
to the top of the output, since those are the highest-priority rows for a
human reviewer — this is how you'd actually generate the "manual audit of
200 Q&A pairs" docs/product.md's Feature 2 acceptance criteria calls for,
instead of assembling it by hand.

Usage:
    python3 scripts/audit_rag.py --n 50 --output audit_report.csv
    python3 scripts/audit_rag.py --n 20 --scheme "Lakshmir Bhandar"
    python3 scripts/audit_rag.py --dry-run   # list sampled Q&A pairs, no DB/model calls
"""
...
async def main_async(n: int, output_path: Path, only_scheme: str | None, dry_run: bool) -> None:
    ...
    # ungrounded results first — highest review priority
