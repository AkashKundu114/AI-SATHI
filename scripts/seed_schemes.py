...
"""
Seed data/schemes/raw/*.pdf -> scheme_documents / scheme_chunks.

FIXED in this pass: the previous version of this script called
`sql_text()` with no argument at both call sites (the INSERT statements
were entirely missing) — every seeding run would raise a TypeError before
writing a single row. That's fixed below; see `_INSERT_DOCUMENT_SQL` and
`_INSERT_CHUNK_SQL`.

Also added in this pass:
  --dry-run     extract + chunk only, no DB/embedding calls, no writes
  --only NAME   process a single PDF by filename instead of the whole dir
  --force       re-seed a PDF that's already been seeded (deactivates the
                old scheme_documents row instead of deleting it, so the
                audit trail in the table is preserved)
  auto-detection of source_language (Bengali char ratio) when a
                manifest entry omits it, instead of silently defaulting
                chunk_bengali to NULL for every chunk
  per-chunk embedding error handling — one bad chunk no longer aborts
                the whole file; failures are counted and reported
  a JSON summary report written next to the PDFs for auditability,
                matching the "don't just print and forget" discipline
...
_BENGALI_LANGUAGE_RATIO_THRESHOLD = 0.15  # conservative: PDFs are often mixed Bengali/English
...
_INSERT_CHUNK_SQL = """
    INSERT INTO scheme_chunks
        (document_id, chunk_text, chunk_bengali, embedding, chunk_index)
    VALUES
        (:document_id, :chunk_text, :chunk_bengali, CAST(:embedding AS vector), :chunk_index)
"""

_DEACTIVATE_EXISTING_SQL = """
    UPDATE scheme_documents SET is_active = FALSE
    WHERE source_file = :source_file AND is_active = TRUE
    RETURNING id
"""

_EXISTING_ACTIVE_SQL = """
    SELECT id FROM scheme_documents WHERE source_file = :source_file AND is_active = TRUE
"""
...
def _looks_bengali(text: str) -> bool:
    """Cheap language auto-detection so a manifest entry that forgets
    'source_language' doesn't silently leave chunk_bengali NULL for every
    chunk (which would make hybrid full-text search on chunk_bengali_tsv
    useless for that document). Same style of char-ratio heuristic already
    used in services/orchestrator/nodes/ledger_node.py::_looks_code_mixed."""
    if not text:
        return False
    sample = text[:2000]  # a prefix is enough to classify a whole document
